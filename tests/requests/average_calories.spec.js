...
describe('api', () => {
  ...
  describe('Test GET /api/v1/recipes/average_calories', () => {
    test('should return average calories for chicken food_type', async () => {
      ...
      return request(app).get('/api/v1/recipes/average_calories').query({
        food_type: 'chicken'
      })
      .then(response => {
        expect(response.status).toBe(200)
        expect(response.body.average).toBe(771)
      })
    })
